...
require_once "updateTeacherListValidate.php";
$detail_t_email=$_SESSION['target_t_email'];
if($_SERVER['REQUEST_METHOD']==='POST'){
if (empty($errors)) {
  $crow=mysqli_query($con,"select * from teacher_table where email='$detail_t_email'");
  if(mysqli_num_rows($crow)>0){
    $result=mysqli_fetch_assoc($crow);
    $filePath = $result['image'];
    if($result['name']!=$sname){
  $sql2= "UPDATE user_type SET name ='$sname' where email='$detail_t_email'";
...
$sql1= "UPDATE teacher_table SET ";
$sql1 .= implode(", ",$updateFields);
$sql1 .= "WHERE email='$detail_t_email'";
